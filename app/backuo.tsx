...
// BASELINE_TAG: pre-feedback-2025-08-10
// CURRENT_TAG: post-feedback-v1-2025-08-10 (TypeScript-typed)
  ...
  return (
    <div id="top" className="min-h-screen text-slate-900" style={{ background: THEME_BG, fontFamily: FONT_FAMILY }}>
      ...
      <header className="sticky top-0 z-40 bg-white/85 backdrop-blur supports-[backdrop-filter]:bg-white/70 border-b border-slate-900/5">
        <div className="mx-auto max-w-7xl px-4 py-5 flex items-center justify-between">
          <a href="#top" className="flex items-center gap-3 rounded-md focus:outline-none focus:ring-2" aria-label="BagTalk — Back to top" style={{ outlineColor: ACCENT }}>
            <LogoMark className="h-9 w-9" style={{ color: ACCENT }} />
            <span className="text-lg md:text-xl font-semibold tracking-tight">BagTalk</span>
          </a>
      ...
      <section className="relative overflow-hidden">
        <div className="mx-auto max-w-7xl px-4 pt-16 pb-12 md:pt-20 md:pb-16 grid md:grid-cols-2 gap-10 items-center">
          <div>
      ...
      <section id="intel" className="py-20 md:py-28">
        <div className="mx-auto max-w-7xl px-4 grid md:grid-cols-2 gap-12 md:gap-16 items-center">
          <div>
          ...
          <div>
            <div className="flex justify-center py-4">
              <PhoneFrame title="App Screen" src={assets.slides[0]} alt="App Screen" className="w-[240px] md:w-[280px]" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-14 md:py-20">
        <div className="mx-auto max-w-7xl px-4 grid md:grid-cols-2 gap-12 items-center">
          {/* Left: two stacked squares (half the phone width) */}
          <div className="flex flex-col items-center gap-2">
            <SquareImage src={assets.slides[1]} alt="Community insights" className="w-[300px] md:w-[315px]" />
            <SquareImage src={assets.slides[2]} alt="Trend signals" className="w-[300px] md:w-[315px]" />
          </div>
          {/* Right: text content */}
          <div>
      ...
      {/* Final CTA / Waitlist */}
      <section ref={waitlistRef} className="py-16 md:py-24">
...
type SquareImageProps = { src?: string; alt?: string; className?: string };
function SquareImage({ src, alt, className = '' }: SquareImageProps): JSX.Element {
  return (
    <div className={`relative aspect-square rounded-2xl overflow-hidden bg-white shadow-[0_8px_30px_rgba(2,6,23,0.06)] ${className}`}>
      <div className="absolute inset-0 rounded-2xl" style={{ boxShadow: 'inset 0 0 0 1px rgba(2,6,23,0.08)' }} />
      {src ? (
        <img
          src={src}
          alt={alt || ''}
        ...
        <div className="absolute inset-0 grid place-items-center text-xs text-slate-500">Image</div>
      )}
    </div>
  );
}
...
function LogoMark({ className = '', style }: IconProps): JSX.Element {
  return (
    <svg viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg" className={className} style={style} role="img" aria-label="BagTalk logo mark">
      <path fill="currentColor" d="M8 10c0-3.314 2.686-6 6-6h20c3.314 0 6 2.686 6 6v16c0 3.314-2.686 6-6 6H18l-8 8v-8c-1.1 0-2-.9-2-2V10z" />
      <rect x="18" y="18" width="18" height="6" rx="3" fill="#ffffff" />
      <rect x="18" y="12" width="6" height="18" rx="3" fill="#ffffff" />
    </svg>
  );
}
